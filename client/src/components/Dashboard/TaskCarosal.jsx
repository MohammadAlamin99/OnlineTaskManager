...
const TaskCarosal = ({ props, onTaskCreated }) => {
  ...
  const onBtnClick = async () => {
    ...
    try {
      ...
      formData.append("dueDate", dueDate ? new Date(dueDate) : "");
      formData.append("createdBy", id);

      // For arrays
      selectedUsers.forEach(user => formData.append("assignTo[]", user));
      ...
      const createTask = await CreateTaskRequest(formData);
          ...
          <div className="mb-3">
            ...
            <div className="input-group mb-2">
              <input
                ...
                name="assets"
                className="form-control"
                onChange={(e) => setAssets(e.target.files[0])}
              />
            </div>
          </div>
