...
import { getTaskRequest, DeleteTaskRequest } from "../apiRequiest/apiRequiest";
...
const TodoTask = () => {
  ...
  const handleUpdateTask = (newTask) => {
    todoDispatch(
      setTodo(
        getTodo.map((task) => (task._id === newTask._id ? newTask : task))
      )
    );
  };
  // delete Task
  const DeleteTaskHandler = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (isDelete) {
          const updatedTask = getTodo.filter((task) => task._id !== id);
          todoDispatch(setTodo(updatedTask));
        }
      }
    });
  };
